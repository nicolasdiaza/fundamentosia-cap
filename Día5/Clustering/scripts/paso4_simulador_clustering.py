"""
===============================================================================
MÓDULO DE CLUSTERING - PASO 4: SIMULADOR VISUAL DE ASIGNACIÓN DE PERFILES
Capacitación de IA - Servicio Nacional de Migración de Panamá (Día 5)
===============================================================================
OBJETIVO:
Simulador interactivo con GUI (Sliders + Identificador de Perfil en vivo)
para ingresar las características de un nuevo pasajero y ver a qué Cluster lo asigna la IA.
===============================================================================
"""
...
# 1. Cargar modelos y datos de historia
...
centroides_reales_hist = scaler.inverse_transform(kmeans.cluster_centers_)
...
cluster_mas_comun = pd.Series(kmeans.labels_).value_counts().idxmax()
...
# 5. Función de actualización del identificador de perfil
def actualizar(val=None):
    ...
    # Escalar y predecir cluster con K-Means
    X_nuevo_scaled = scaler.transform(df_nuevo)
    cluster_asignado = kmeans.predict(X_nuevo_scaled)[0]
    
    # Calcular distancia al centroide del cluster asignado
    centroide_scaled = kmeans.cluster_centers_[cluster_asignado]
    distancia = np.linalg.norm(X_nuevo_scaled[0] - centroide_scaled)
