...
N_HISTORIA = preguntar_entero("\n¿Cuántos registros para el dataset de historia (entrenamiento)?", 400)
N_EVALUACION = preguntar_entero("¿Cuántos registros para el dataset de evaluación final?", 100)

# Paso 1: dataset de historia (para que el modelo descubra los perfiles) — 3 grupos generados
...
# Paso 2: dataset de evaluación — cohorte de viajeros INDEPENDIENTE, no una porción de la
...
# Esto obliga al modelo a generalizar sobre datos genuinamente nuevos, no solo a
# repetir un patrón que ya vio en el mismo lote de generación.
np.random.seed(7)
TOTAL_EVALUACION = N_EVALUACION
...
# Paso 3: definir rutas y guardar los CSV
