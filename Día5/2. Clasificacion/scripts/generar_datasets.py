import os
import pandas as pd
import numpy as np
from sklearn.model_selection import train_test_split


def preguntar_entero(mensaje, por_defecto):
    texto = input(f"{mensaje} [Enter = {por_defecto}]: ").strip()
    if texto == "":
        return por_defecto
    if texto.isdigit() and int(texto) > 0:
        return int(texto)
    print(f"  Entrada no válida, se usará el valor por defecto ({por_defecto}).")
    return por_defecto


np.random.seed(42)

# Paso 1: configuración interactiva del tamaño de los datasets
print("--- CONFIGURACIÓN DE LOS DATASETS ---")
N_HISTORIA = preguntar_entero("\n¿Cuántos registros para el dataset de entrenamiento (historia)?", 400)
N_EVALUACION = preguntar_entero("¿Cuántos registros para el dataset de evaluación final?", 100)
TOTAL_REGISTROS = N_HISTORIA + N_EVALUACION

# Paso 2: generar variables predictoras realistas para un Puesto Migratorio
pasajeros = np.random.randint(500, 3500, size=TOTAL_REGISTROS)
vuelos_simultaneos = np.random.randint(2, 18, size=TOTAL_REGISTROS)
inspectores_disponibles = np.random.randint(5, 35, size=TOTAL_REGISTROS)
porcentaje_escaneo = np.round(np.random.uniform(10, 90, size=TOTAL_REGISTROS), 1)

# Paso 3: regla operativa de congestión con ruido realista
# COLCHON_OPERATIVO hace que, en la mayoría de los turnos, la operación sea Normal
# (la Alerta de Congestión es el evento MINORITARIO, no la norma).
COLCHON_OPERATIVO = 1400
capacidad_atencion = inspectores_disponibles * 105 - (vuelos_simultaneos * 40) - (porcentaje_escaneo * 5) + COLCHON_OPERATIVO
diferencia_demanda = pasajeros - capacidad_atencion

# 0 = Operación Normal (Capacidad suficiente) | 1 = Alerta de Congestión (Demanda supera capacidad)
probabilidad_congestion = 1 / (1 + np.exp(-diferencia_demanda / 300))
estado_operacion = (np.random.rand(TOTAL_REGISTROS) < probabilidad_congestion).astype(int)

df_completo = pd.DataFrame({
    'pasajeros_proyectados': pasajeros,
    'vuelos_simultaneos': vuelos_simultaneos,
    'inspectores_disponibles': inspectores_disponibles,
    'porcentaje_escaneo': porcentaje_escaneo,
    'alerta_congestion': estado_operacion
})

# Paso 4: dividir en historia y evaluación, manteniendo la proporción de clases (stratify).
# Con TOTAL_REGISTROS muy pequeño (<~30), stratify puede fallar si una clase queda sin
# representación suficiente en alguna de las dos particiones; en ese caso se usa un split simple.
try:
    df_historia, df_evaluacion = train_test_split(
        df_completo,
        train_size=N_HISTORIA,
        test_size=N_EVALUACION,
        random_state=42,
        stratify=df_completo['alerta_congestion']
    )
except ValueError:
    print("  No hay suficientes registros de la clase minoritaria para mantener las proporciones; "
          "se usará una división simple (sin stratify).")
    df_historia, df_evaluacion = train_test_split(
        df_completo,
        train_size=N_HISTORIA,
        test_size=N_EVALUACION,
        random_state=42
    )

# Paso 5: definir rutas y guardar los CSV
script_dir = os.path.dirname(os.path.abspath(__file__))
dir_datos = os.path.join(script_dir, "..", "datos")
os.makedirs(dir_datos, exist_ok=True)

path_historia = os.path.join(dir_datos, "dataset_historia_clasificacion.csv")
path_eval = os.path.join(dir_datos, "dataset_evaluacion_clasificacion.csv")

df_historia.to_csv(path_historia, index=False)
df_evaluacion.to_csv(path_eval, index=False)

print("="*75)
print(f" Datasets generados: {len(df_historia)} filas (historia) + {len(df_evaluacion)} filas (evaluación)")
print(" -> Ejecuta 'paso1_carga_y_exploracion.py' para ver el detalle gráfico completo.")
print("="*75)
